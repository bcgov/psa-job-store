...
async function executeQueriesAndWriteCSVs(client: Client, queries: Set<string>, startDate: string, endDate: string) {
  ...
  for (const queryFile of queries) {
    ...
    const outputPath = path.join(isLocal ? './tmp' : '/tmp', outputFileName);
  ...
  for (const { email, reports } of recipientConfigs) {
    ...
    for (const report of reports) {
      ...
      if (outputPath && fs.existsSync(outputPath)) {
        console.log(`Attaching report ${report} for ${email}`);
        attachments.push({
          filename: path.basename(outputPath),
          path: outputPath,
        });
      } else {
        console.log(`Report ${report} not found or no data. Skipping attachment for ${email}.`);
      }
    }
    ...
    if (attachments.length > 0) {
      const mailOptions = {
        ...
        to: `${email}@gov.bc.ca`,
        subject: `${moment(startDate).format('MMMM')} Monthly Report`,
      ...
      console.log(`No attachments to send to ${email}. Skipping email.`);
    }
  }
}
