...
export const convertData = (nodes: Node[], edges: Edge[]) => {
  ...
  // Add this new function to calculate levels and store nodes by level
  const calculateLevels = (nodeId: string, level: number) => {
    ...
    if (node) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      node.children.forEach((childId: any) => calculateLevels(childId, level + 1));
    }
  };
  ...
  const sortNodes = (nodeIds: any[]) => {
    return nodeIds.sort((aId, bId) => {
      ...
      // If nodes are on the same level, sort by x-coordinate
      if (a.level === b.level) {
        // console.log('a, b: ', a.position.x, b.position.x);
        return a.position.x - b.position.x;
      }

      // If nodes are on different levels, maintain the existing sorting logic
  ...
  // Sort root nodes
  const sortedRootIds = sortNodes(rootNodes.map((node) => node.id));

  // Recursively sort all nodes
  sortedRootIds.forEach(sortChildren);

  // Convert the map to a sorted array, starting with root nodes
  const sortedNodes = [...sortedRootIds];
  const addChildrenRecursively = (nodeId: string) => {
    const node = nodeMap.get(nodeId);
    if (node && node.children) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      node.children.forEach((childId: any) => {
        sortedNodes.push(childId);
        addChildrenRecursively(childId);
      });
    }
  };
  sortedRootIds.forEach(addChildrenRecursively);

  // Return the sorted array of nodes
  return sortedNodes.map((id) => nodeMap.get(id));
};
