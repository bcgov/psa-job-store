...
export const TotalCompApprovedRequestPage = () => {
  ...
  const submissionDetailsItems = [
    ...
    },
    {
      ...
      children: <div>{data?.positionRequest?.position_number}</div>, // this is wrong, need to get position number from position request, not job profile
      //children: <div>{data?.positionRequest?.parent_job_profile?.number}</div>, // this is wrong, need to get position number from position request, not job profile
      span: { xs: 24, sm: 24, md: 24, lg: 12, xl: 12 },
    },
  ];
